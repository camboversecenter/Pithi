...
import { 
    ...
    getGuests, getInvitationTemplates, getCeremonyById, isCeremonyExpired
} from '../services/dataService';
import { uploadImage, deleteImage } from '../services/storageService';
import { Ceremony, UserRole, Guest, InvitationTemplate } from '../types';
import { Button, Pagination } from '../components/UIComponents';
import { CalendarClock, History, Plus, Calendar, MapPin, Edit2, Trash2, Image as ImageIcon, ArrowLeft, Briefcase, Eye, Loader2 } from 'lucide-react';
...
import OrganizerGuests from '../components/Organizer/OrganizerGuests';
import OrganizerInvitations from '../components/Organizer/OrganizerInvitations';
import OrganizerPlan from '../components/Organizer/OrganizerPlan';
import OrganizerCeremonyForm from '../components/Organizer/OrganizerCeremonyForm';
...
const OrganizerPortal = () => {
    ...
    const [activeDetailTab, setActiveDetailTab] = useState<'OVERVIEW' | 'GUESTS' | 'INVITATIONS' | 'ANNOUNCEMENTS' | 'PLAN'>('OVERVIEW');
    ...
    const loadCeremonies = async () => {
        ...
        setIsLoading(true);
        try {
            const res = await getCeremonies(user.id, UserRole.ORGANIZER, page, 9, timeFilter);
    ...
    const loadCeremonyDetail = async (id: string) => {
        ...
        try {
            ...
            const [g, tmpl] = await Promise.all([
                getGuests(id, 1, 1000), 
    ...
    const handleSaveCeremony = async (data: Partial<Ceremony>, bannerFile: File | null, khqrFile: File | null) => {
        ...
        try {
            ...
            const payload = {
                organizerId: user.id,
                ownerId: (data.ownerId && data.ownerId.trim() !== '') ? data.ownerId : user.id,
                ...data,
                type: ceremonyType,
                bannerUrl,
                khqrUrl
        ...
        return (
            <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-20">
                <div className="flex justify-between items-center">
                    <Button variant="ghost" onClick={() => { setSearchParams({}); setSelectedCeremony(null); }} className="pl-0 text-slate-500 hover:bg-transparent hover:text-blue-600">
                ...
                <div className="flex overflow-x-auto pb-2 space-x-2 no-scrollbar">
                    {['OVERVIEW', 'GUESTS', 'INVITATIONS', 'ANNOUNCEMENTS', 'PLAN'].map((tab) => (
                        <button
                            key={tab}
                ...
                <div className="min-h-[400px]">
                    ...
                    {activeDetailTab === 'ANNOUNCEMENTS' && (
                        <AnnouncementsPanel
                            ...
                            recipientHint={`ភ្ញៀវ និងម្ចាស់កម្មវិធី « ${selectedCeremony.title} » នឹងទទួលបានការជូនដំណឹងភ្លាមៗ។`}
                        />
                    )}

                    {activeDetailTab === 'PLAN' && (
                        <OrganizerPlan ceremony={selectedCeremony} />
                    )}
                </div>
            </div>
        );
    ...
    return (
        <div className="space-y-8 animate-fade-in-up">
            <div>
                <div className="flex flex-wrap items-center gap-4 mb-2">
                    <h1 className="text-4xl font-bold font-serif text-slate-900 flex items-center gap-3 tracking-tight">
                        <Briefcase className="text-blue-600 w-8 h-8"/> គ្រប់គ្រងកម្មវិធី
                    </h1>
                    <Button onClick={handleCreate} className="shadow-lg shadow-blue-200 bg-blue-600 hover:bg-blue-700 border-transparent transform hover:-translate-y-0.5 transition-all rounded-full px-6">
                        <Plus size={20} className="mr-2"/> បង្កើតកម្មវិធីថ្មី
                    </Button>
                </div>
                <p className="text-slate-500 text-lg">បង្កើត និងគ្រប់គ្រងកម្មវិធីសម្រាប់អតិថិជនរបស់អ្នក។</p>
            </div>
                ...
                {([['UPCOMING', 'ខាងមុខ'], ['PAST', 'កន្លងផុត'], ['ALL', 'ទាំងអស់']] as const).map(([value, label]) => (
                    <button
                        ...
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold transition-all ${
                            timeFilter === value ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                        }`}
                    >
            ...
            {isLoading ? (
                <div className="flex justify-center py-20"><Loader2 className="animate-spin w-10 h-10 text-blue-300"/></div>
            ) : (
                    ...
                    {ceremonies.length > 0 ? ceremonies.map(c => (
                        ...
                        >
                            ...
                            <div className="p-6 flex-1 flex flex-col">
                                <h3 className="text-xl font-bold text-slate-900 font-serif mb-2 line-clamp-1 group-hover:text-blue-600 transition-colors">{c.title}</h3>
                                <div className="space-y-2 text-sm text-slate-500 mb-6 flex-1">
                                    <div className="flex items-center"><Calendar size={14} className="mr-2 text-blue-400"/> {c.date}</div>
                                    <div className="flex items-center"><MapPin size={14} className="mr-2 text-blue-400"/> {c.location || 'មិនទាន់កំណត់ទីតាំង'}</div>
                                ...
                                <div className="mt-auto pt-4 border-t border-slate-50 flex items-center gap-2">
                                    <Button className="flex-1 text-xs shadow-md shadow-blue-100 bg-blue-600 hover:bg-blue-700 border-transparent" onClick={() => handleViewDetail(c)}>
